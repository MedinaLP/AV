...
def reduce_noise(y, sr, prop_decrease=0.85, n_fft=2048, hop_length=512):
...
with tabs[1]:
    ...
    if file is not None:
        ...
        y = normalize_audio(librosa.load(file, sr=None, mono=True)[0])
        ...
        if st.button("🚀 Clean up audio", use_container_width=True):
            with st.spinner("Processing…"):
                st.markdown("#### 🎚️ Advanced Noise Reduction Settings (Optional)")

                col1, col2 = st.columns(2)
                with col1:
                    prop_decrease = st.slider("Noise Reduction Strength (prop_decrease)", 0.1, 1.0, 0.85, 0.05)
                    fft_size = st.selectbox("FFT Size (n_fft)", [1024, 2048, 4096], index=1)
                with col2:
                    hop_length = st.selectbox("Hop Length", [256, 512, 1024], index=1)

                y_clean = reduce_noise(y, sr, prop_decrease=prop_decrease, n_fft=fft_size, hop_length=hop_length)
